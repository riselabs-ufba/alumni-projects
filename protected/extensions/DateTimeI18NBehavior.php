    ...
    public function beforeSave($event) {
        ...
        foreach ($event->sender->tableSchema->columns as $columnName => $column) {
            ...
            if (($column->dbType == 'date')) {
                $event->sender->$columnName = date($this->dateOutcomeFormat, CDateTimeParser::parse($event->sender->$columnName, 'MM/dd/y'));
            } else {
        ...
        foreach ($event->sender->tableSchema->columns as $columnName => $column) {
            ...
            if ($column->dbType == 'date') {
                $event->sender->$columnName = Yii::app()->dateFormatter->formatDateTime(
                        CDateTimeParser::parse($event->sender->$columnName, Yii::app()->locale->dateFormat), 'medium', null);
            } else {                
                $datetime = DateTime::createFromFormat($this->dateTimeOutcomeFormat, $event->sender->$columnName);
                $event->sender->$columnName = $datetime->format('m/d/Y h:i:s');                
            }
        }
